  ...
  const shareGifFile = async () => {
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        toast.success("GIF shared successfully!");
      } else {
        toast.error("Your browser doesn’t support file sharing.");
      }
    } catch (err) {
      console.error("Error sharing file:", err);
      toast.error("Failed to share GIF file!");
    }
    setShowShareOptions(false);
  };

  const shareGifLink = async () => {
    try {
      if (navigator.share) {
        await navigator.share({
          title: gif.title,
        ...
        toast.success("GIF link shared successfully!");
      } else {
      ...
      toast.error("Failed to share GIF link!");
    }
    setShowShareOptions(false);
  };
        ...
        <div className="flex gap-6">
          <div className="w-full sm:w-3/4">
            ...
            <div className="flex sm:hidden gap-1">
              ...
              <button className="ml-auto" onClick={() => setShowShareOptions(true)}>
        ...
        {showShareOptions && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-80">
              <h2 className="text-lg font-bold mb-4">Share this GIF</h2>
              <div className="flex flex-col gap-3">
                <button
                  onClick={shareGifFile}
                  className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600"
                >
                  Share GIF File
                </button>
                <button
                  onClick={shareGifLink}
